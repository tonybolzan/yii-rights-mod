	...
	*/
	public function actionUpdate()
	{
		...
		$childSelectOptions = Rights::getParentAuthItemSelectOptions($model, $type, $exclude);
                ...
                // Edited by admin@tonybolzan.com, to dropdownlist with multiple
                if ($childSelectOptions !== array()) {
                    ...
                    if (isset($_POST['AuthChildForm']) === true) {
                        ...
                        try {
                            ...
                            foreach ($_POST['AuthChildForm']['itemname'] as $AuthChildFormItemname) {
                                $childFormModel->itemname = $AuthChildFormItemname;
                                ...
                                if ($childFormModel->validate() === true) {
                                    ...
                                    $childrens[] = $child->getNameText();
                                } else {
                                    throw new CException(Rights::t('core','Error creating childrens'));
                                }
                            }

                            $transaction->commit();
                            // Set a flash message for adding the child
                            Yii::app()->user->setFlash($this->module->flashSuccessKey, Rights::t('core', 'Childrens :name added.', array(':name' => implode(', ', $childrens))));
                            // Reidrect to the same page
                            $this->redirect(array('authItem/update', 'name' => urlencode($itemName)));
                        } catch (CException $e) {
                            $transaction->rollback();
                            Yii::app()->user->setFlash($this->module->flashErrorKey, $e->getMessage());
                        }
                    }
                } else {
                    $childFormModel = null;
