...
class RGenerator extends CApplicationComponent
{
	...
	* Returns all controllers under the specified path.
        * PS.: Edited by admin@tonybolzan.com, to make prefix in subfolder controllers
	* @param string $path the path.
	* @return array the controllers.
	*/
	protected function getControllersInPath($path, $prefix=null)
	{
		$controllers = array();
		...
		if( file_exists($path)===true )
		{
			...
			foreach( $controllerDirectory as $entry )
			{
				if( $entry{0}!=='.' )
				{
					...
					{
						$name = (is_null($prefix) ? '' : $prefix .'/') . substr($entry, 0, -14);
					...
					if( is_dir($entryPath)===true )
                                        {
                                                $prefixPath = (is_null($prefix) ? ucfirst($entry) : $prefix .'/'. ucfirst($entry));
                                                
						foreach( $this->getControllersInPath($entryPath,$prefixPath) as $controllerName=>$controller )
